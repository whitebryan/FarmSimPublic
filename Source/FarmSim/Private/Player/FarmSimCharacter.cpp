...
#define SurfaceTypeGrass EPhysicalSurface::SurfaceType3
#define SurfaceTypeSand EPhysicalSurface::SurfaceType4
#define SurfaceTypeRock EPhysicalSurface::SurfaceType5
...
void AFarmSimCharacter::BeginPlay()
{
	...
	const float curSens = getSensitivity();
	setSensitivity(curSens);

	const int FOV = getFOV();
	setFOV(FOV);
}
...
void AFarmSimCharacter::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherComp->ComponentHasTag("Interaction"))
	{
		...
		//OtherActor->GetComponentsByInterface() 
...
void AFarmSimCharacter::Tick(float DeltaTime)
{
	...
	{
		FHitResult RV_Hit;
		RV_Hit = placementLineTrace(false, true);
		...
		else if (IsValid(RV_Hit.GetActor()) && (surfaceHit == SurfaceTypeGround || surfaceHit == SurfaceTypeGrass || surfaceHit == SurfaceTypeSand || surfaceHit == SurfaceTypeRock) && curTool.MatchesTagExact(FGameplayTag::RequestGameplayTag("PlayerToolStatus.Shovel")))
		{
			if (placementPreview->GetClass()->ImplementsInterface(UItemHighlightInterface::StaticClass()))
			{
		...
		else if(!placementPreview->IsHidden())
		{
	...
	else if (curFishingIndicator && findTagOfType(playerStatusTag).MatchesTagExact(FGameplayTag::RequestGameplayTag("PlayerStatus.FishingCasting")))
	{
		...
		{
			FHitResult findGround = placementLineTrace(false, false, false, curDistance, 2000);
...
{
	GetFollowCamera()->SetFieldOfView(FMath::Clamp(newFOV, 30, 130));
...
void AFarmSimCharacter::CameraAction(FVector2D Value)
{
	...
	else
	{
		...
		{
			AddControllerYawInput(Value.X * getSensitivity());
			AddControllerPitchInput(Value.Y * getSensitivity());
		}
	}

}

void AFarmSimCharacter::MoveAction(FVector2D Value)
{
	if(!toolUsed && findTagOfType(playerStatusTag).MatchesAnyExact(movementAllowableTags))
	{
		Super::MoveAction(Value);
	}
}

void AFarmSimCharacter::JumpAction(bool isJumping)
{
	if (findTagOfType(playerStatusTag).MatchesAnyExact(movementAllowableTags))
	{
		Super::JumpAction(isJumping);
	}
}
...
void AFarmSimCharacter::UseToolAction_Implementation()
{
	...
	{
		FHitResult RV_Hit = placementLineTrace(false, true);
		...
		else if (surfaceHit == SurfaceTypeGround || surfaceHit == SurfaceTypeGrass || surfaceHit == SurfaceTypeRock)
		{
			...
			else
			{
				toolUsed = true;
				FVector placementPoint = RV_Hit.ImpactPoint;
				placementPoint.Z += growthPlotZExtent/2;

				FActorSpawnParameters myParams;
				myParams.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;//DontSpawnIfColliding;
...
void AFarmSimCharacter::FishingCastAction_Implementation(bool pressed)
{
	...
	if (pressed && curStatus.MatchesTagExact(FGameplayTag::RequestGameplayTag("PlayerStatus.Normal")))
	{
		...
		FHitResult findGround = placementLineTrace(false, false);
...
FHitResult AFarmSimCharacter::placementLineTrace(bool fromCamera, bool snapToGrid, bool drawDebug, float distanceMod, float distanceDown)
{
	...
	if (fromCamera)
	{
		startPoint = GetFollowCamera()->GetComponentLocation();
		endPoint = startPoint + (GetFollowCamera()->GetForwardVector() * 1000);
	}
	else
	{
		startPoint = GetActorLocation() + (GetActorForwardVector() * digDistance * distanceMod);
		endPoint = startPoint;
	...
	if (hit)
	{
		return placementLineTraceDown(RV_Hit.ImpactPoint, snapToGrid, drawDebug, distanceMod, distanceDown);
	}
	else
	{
		FVector placementPoint;
		if (fromCamera)
		{
			placementPoint = GetActorLocation() + (GetFollowCamera()->GetForwardVector() * (digDistance * distanceMod));
		}
		else
		{
			placementPoint = GetActorLocation() + (GetActorForwardVector() * (digDistance * distanceMod));
		}

		return placementLineTraceDown(placementPoint, snapToGrid, drawDebug, distanceMod, distanceDown);
	}
}

//Reuseable line trace down digDistance in front of player and snapped to grid
FHitResult AFarmSimCharacter::placementLineTraceDown(FVector point, bool snapToGrid, bool drawDebug, float distanceMod, float distanceDown)
{
	FHitResult RV_Hit(ForceInit);
	FCollisionQueryParams RV_TraceParams = FCollisionQueryParams(FName(TEXT("RV_Trace")), false, this);
	RV_TraceParams.bTraceComplex = true;
	...
	float maxDist = FVector::Distance(GetActorLocation(), (GetActorForwardVector() * digDistance * distanceMod));
	if (dist > maxDist)
	{
		startPoint = GetActorLocation() + ((GetActorLocation() - point) * digDistance);
	}
	else
	{
		startPoint = point;
...
{
	FHitResult RV_Hit = placementLineTrace();
	...
	if (place)
	{
		...
		else
		{
			...
			if (RV_HitForward.bBlockingHit || (surfaceHit != SurfaceTypeGround && surfaceHit != SurfaceTypeGrass && surfaceHit != SurfaceTypeSand && surfaceHit != SurfaceTypeRock))
			{
